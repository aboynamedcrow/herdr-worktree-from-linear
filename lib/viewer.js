...
export function renderWithGlow(markdown, fallback, { timeoutMs = RENDER_TIMEOUT_MS } = {}) {
  ...
  if (!reaperWired) {
    reaperWired = true;
    process.on('exit', () => {
      for (const renderer of running) kill(renderer, 'SIGKILL');
    });
  }
  let child = null;
  let restart = false;
  ...
  const render = () => {
    ...
    running.add(started);
    let hard = null;
    // Presentation may finish before a timed-out child exits. Keep ownership
    // through the kill grace and any later render until its actual close.
    started.once('close', () => {
    ...
    deadline = setTimeout(() => {
      ...
      hard = setTimeout(() => kill(started, 'SIGKILL'), KILL_GRACE_MS);
