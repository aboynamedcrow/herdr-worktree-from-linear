...
function startAgent(issue, { env, exec, log }) {
  let context;
  try { context = JSON.parse(env.HERDR_PLUGIN_CONTEXT_JSON); } catch { /* checked below */ }
  const paneId = env.HERDR_WFP_START_PANE ?? context?.focused_pane_id;
  ...
  const result = exec(task, ['new', `${issue.identifier} [M]`], {
    env: { ...env, HERDR_ENV: '1', HERDR_PANE_ID: paneId }, timeout: 600000,
  });
