...
export function createSocketHome({
  ...
  let dir;
  try {
    dir = realpathSync(mkdtemp(join(base, 'wfl-')));
    chmodSync(dir, 0o700);
  } catch (err) {
  ...
  // The exclusive random directory supplies uniqueness. Keep the basename short:
  // native Herdr 0.9 metadata truncates values to 80 characters, including on macOS.
...
export function publishHost(host, { herdrBin, exec = spawnSyncExec }) {
  if (Buffer.byteLength(host.socketPath, 'utf8') > 80 || /[\x00-\x1f\x7f]/.test(host.socketPath)) {
    return { ok: false, error: 'issue host socket path exceeds native metadata limits; set a shorter TMPDIR or XDG_RUNTIME_DIR' };
  }
