...
export function openPickerArgs(pluginId = 'tdi.worktree-from-linear', cwd, placement = DEFAULT_PLACEMENT, { width = DEFAULT_POPUP.width, height = DEFAULT_POPUP.height, targetPane, start = false } = {}) {
  ...
  // Keep the invoking pane when Herdr gives the picker its own pane context.
  if (start) args.push('--env', 'HERDR_WFP_MODE=start', '--env', `HERDR_WFP_START_PANE=${targetPane}`);
  return args;
}
