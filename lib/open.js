...
// Two spellings of one directory have to compare equal, but an unresolvable
// path must never resolve to something else: it is compared as it came, which
// simply fails to match. Same rule as lib/slot.js uses for the checkout path.
...
export function openPicker(env = process.env, exec = runCmd, { start = false } = {}) {
  ...
  // A directory that cannot be resolved — the pane's cwd was removed under it —
  // simply fails to match, and says so in this plugin's own words. Resolving it
  // unguarded would surface a raw ENOENT as the user-facing diagnostic and as
  // the notification body instead.
  if (pane?.pane_id !== paneId || pane?.workspace_id !== workspaceId
      || typeof actual !== 'string' || canonical(actual) !== canonical(cwd)) {
  ...
  const args = openPickerArgs('tdi.worktree-from-linear', cwd, placement, { ...size, targetPane: paneId, start });
