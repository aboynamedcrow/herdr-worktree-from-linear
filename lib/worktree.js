...
export function readWorktreePlan(stdout) {
  ...
  for (const candidate of plan.candidates) {
    ...
    if (candidate.source !== undefined && (!['local', 'remote', 'new'].includes(candidate.source)
        || !['NONE', 'UNKNOWN', 'OPEN', 'CLOSED', 'MERGED'].includes(candidate.pr_state)
        || !text(candidate.action) || typeof candidate.recommended !== 'boolean'
        || !/^(?:[a-f0-9]{40}|[a-f0-9]{64})$/.test(candidate.start_commit)
        || (candidate.source === 'local') !== candidate.existing)) {
      throw new Error('worktree-from-linear: invalid Plus branch lifecycle choice');
    }
  }
  return plan;
}
