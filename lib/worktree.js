...
// No --json: herdr's worktree commands do not take it, and an unknown option is a usage
// error (status 2), not a flag the server ignores. JSON is what these commands print
// anyway, so there was never anything to ask for.
export function buildWorktreeArgs(exists, repoRoot, branchName, baseRef) {
  if (exists) {
    return ['worktree', 'open', '--cwd', repoRoot, '--branch', branchName, '--focus'];
  }
  return ['worktree', 'create', '--cwd', repoRoot, '--branch', branchName, '--base', baseRef, '--focus'];
}
