...
function hostIdentity(proc, hostScript) {
  ...
  // Node resolves module paths, while native process argv preserves a plugin
  // link's spelling. Resolve an absolute script path without guessing the
  // foreground process's cwd; a different or unreadable script stays refused.
  if (argv[1] !== hostScript) {
    if (!isAbsolute(argv[1])) return null;
    try {
      if (realpathSync(argv[1]) !== hostScript) return null;
    } catch {
      return null;
    }
  }
...
export function selectSlot(tabs, panes, workspaceId, tabLabel, paneLabel, binding = null) {
  ...
  if (binding) {
    const tab = tabs.find((t) => t.tab_id === binding.tabId);
    const pane = panes.find((p) => p.pane_id === binding.paneId);
    if (!tab || !pane || pane.tab_id !== tab.tab_id) {
      return { ok: false, error: 'the recorded Crew issue slot is absent or moved' };
    }
...
export function slotSettings(config = {}) {
  const byId = config.issueSlotById === true;
...
function inspect(exec, herdrBin, workspaceId, tabLabel, paneLabel, timeoutMs, byId = false) {
  let binding = null;
  if (byId) {
    const got = callJson(exec, herdrBin, ['workspace', 'get', workspaceId], { timeoutMs });
    const workspace = got.result?.workspace;
    const tokens = workspace?.tokens ?? {};
    if (!got.ok || workspace?.workspace_id !== workspaceId
        || typeof tokens !== 'object' || Array.isArray(tokens)) {
      return { ok: false, error: 'the workspace has no recorded Crew issue slot' };
    }
    if (Object.keys(tokens).some((key) => key.startsWith('crew_'))) {
      if (!typable(tokens.crew_crew_tab) || !typable(tokens.crew_issue_pane)) {
        return { ok: false, error: 'the workspace has no recorded Crew issue slot' };
      }
      binding = { tabId: tokens.crew_crew_tab, paneId: tokens.crew_issue_pane };
    } else if (!typable(tabLabel) || !typable(paneLabel)) {
      return { ok: false, error: 'the workspace has no recorded Crew issue slot or explicit legacy labels' };
    }
  }
  ...
  return { ...selectSlot(tabs.result.tabs, panes.result.panes, workspaceId, tabLabel, paneLabel, binding), byId: binding !== null };
}
...
function examineHost(exec, herdrBin, target, expected, timeoutMs, hostScript) {
  ...
  if (meta.workspaceId !== expected.workspaceId || meta.tabId !== target.tabId
      || (!expected.byId && meta.label !== expected.paneLabel)) {
...
export async function deliverIssueDetails({
  ...
  const expected = { workspaceId, checkoutPath, paneLabel: settings.paneLabel, byId: settings.byId };
  ...
  let first;
  let target;
  // The startup command can still be publishing metadata after the pane appears.
  // Wait for that host to be ready within the same layout deadline. These are
  // read-only probes; nothing launches or sends a request during settling.
  for (;;) {
    const remaining = deadline - now();
    if (remaining <= 0) break;
    found = inspect(exec, herdrBin, workspaceId, settings.tabLabel, settings.paneLabel, remaining, settings.byId);
    if (found.ok) {
      target = { paneId: found.paneId, tabId: found.tabId };
      expected.byId = found.byId;
      const left = deadline - now();
      if (left <= 0) break;
      first = examineHost(exec, herdrBin, target, expected, Math.min(settings.commandMs, left), hostScript);
      if (first.ok || !first.hostless) break;
    }
    const left = deadline - now();
    if (left <= 0) break;
  ...
  if (!first) return { ok: false, error: `the issue host in ${paneId} did not become ready within ${settings.settleMs}ms` };
  ...
  const again = inspect(exec, herdrBin, workspaceId, settings.tabLabel, settings.paneLabel, ms, settings.byId);
  if (!again.ok) return { ok: false, error: again.error };
  if (again.byId !== expected.byId) {
    return { ok: false, error: 'the Crew binding mode changed while preparing' };
  }
