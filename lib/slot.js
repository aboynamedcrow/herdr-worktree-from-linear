...
function inspect(exec, herdrBin, workspaceId, tabLabel, paneLabel, timeoutMs, byId = false) {
  ...
  if (byId) {
    ...
    if (!got.ok || workspace?.workspace_id !== workspaceId
        || !typable(tokens?.crew_crew_tab) || !typable(tokens?.crew_issue_pane)) {
  ...
  return selectSlot(tabs.result.tabs, panes.result.panes, workspaceId, tabLabel, paneLabel, binding);
}
