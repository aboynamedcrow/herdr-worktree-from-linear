...
// protocol-22 API schema — not inferred from a sibling command. It answers a server
// error as JSON on stderr with status 1 and a usage error with status 2. Commands
// that only act (run, report-metadata) may print nothing at all, so those are judged
// by exit status alone.
//
// The pane and tab commands here print JSON with no flag to ask for it, so none is
// passed. That is a fact about these commands, not about the whole CLI: `plugin list`
// does take `--json` (verified against installed herdr 0.9.0), and lib/worktree.js
// passes it there. Check the subcommand's own parser before adding or removing it.
